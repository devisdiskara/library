...
if (isset($_POST['btnSimpan'])) {
    ...
    if ($gambar_sampul['error'] === 0) {
        ...
        if (move_uploaded_file($tmpFileGambar, $dirGambar . $randomGambar . '_' . $namaFileGambar)) {
            ...
            header("Location: dashboard.php?page=buku&status=upload_gagal");
            exit;
        }
    } else {
    ...
    $query = "INSERT INTO buku (id_kategori, judul, pengarang, deskripsi, gambar_sampul, path_file, link_shopee, link_tokopedia) 
              VALUES ('$id_kategori', '$judul', '$pengarang', '$deskripsi', '$gambar_sampul', '$path_file', '$link_shopee', '$link_tokopedia')";
    $result = mysqli_query($koneksi, $query);

    if ($result) {
        echo "<script>alert('Data buku berhasil disimpan'); document.location='dashboard.php?page=buku&status=sukses';</script>";
    } else {
        echo "<script>alert('Gagal menyimpan data buku ke database');</script>";
        header("Location: dashboard.php?page=buku&status=gagal");
    }
}
...
if (isset($_POST['btnUbah'])) {
    ...
    if ($gambar['error'] === 4) {
        ...
        if ($result) {
            echo "<script>alert('Data buku berhasil diubah'); document.location='dashboard.php?page=buku&status=ubah_sukses';</script>";
        } else {
            echo "<script>alert('Gagal mengubah data buku');</script>";
            header("Location: dashboard.php?page=buku&status=ubah_gagal");
        }
    } else {
...
if (isset($_POST['btnHapus'])) {
    ...
    if ($hapus) {
        echo "<script>alert('Data buku berhasil dihapus'); document.location='dashboard.php?page=buku&status=hapus_sukses';</script>";
    } else {
        echo "<script>alert('Data buku gagal dihapus'); document.location='dashboard.php?page=buku&status=hapus_gagal';</script>";
    }
}
?>